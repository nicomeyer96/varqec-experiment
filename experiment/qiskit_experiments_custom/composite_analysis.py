...
####################################################################################################
# Modifications:                                                                                   #
# - Added 'resample' flag to _component_experiment_data method()                                   #
# - Adapted _marginalized_component_data():                                                        #
#   - removed _format_memory call (not required for tomography, not supported for IQM experiments) #
#   - removed logging of memory (not required for tomography, not supported for IQM experiments)   #
#   - added possibility for cluster re-sampling                                                    #
# - Added `_resample` method that performs cluster re-sampling on bistring measurements            #
####################################################################################################
...
class CompositeAnalysis(BaseAnalysis):
    ...
    """
    Modified: Added flag to activate cluster re-sampling, propagated to `_marginalized_component_data`
    """
    def _component_experiment_data(self, experiment_data: ExperimentData, resample: bool = False) -> List[ExperimentData]:
        ...
        marginalized_data = self._marginalized_component_data(experiment_data.data(), resample=resample)
    ...
    """
    Modified: Added flag to activate cluster re-sampling
    """
    def _marginalized_component_data(self, composite_data: List[Dict], resample: bool = False) -> List[List[Dict]]:
        ...
        for datum in composite_data:
            ...
            """
            Modified: We do not need this information for state tomography, breaks extraction of IQM data
            """
            ...
            for i, index in enumerate(metadata["composite_index"]):
                ...
                """
                Modified: Support IQM results and allow for cluster re-sampling
                """
                if "counts" in datum:   # TODO
                    if composite_clbits is not None:
                        sub_data_counts = marginal_distribution(
                            counts=datum["counts"],
                            indices=composite_clbits[i],
                        )
                    else:
                        sub_data_counts = datum["counts"]
                    # perform re-sampling
                    if resample:
                        sub_data["counts"] = self._resample(sub_data_counts)
                    else:
                        sub_data["counts"] = sub_data_counts

                """
                Modified: We do not need this information for state tomography
                """
                # if "memory" in datum:
                #     if composite_clbits is not None:
                #         # level 2
    ...
    """
    Modified: Method for performing cluster re-sampling on bitstring measurements
    """
    @staticmethod
    def _resample(data: dict) -> dict:
        # extract bitstring and respective measurement counts
        keys, values = list(data.keys()), list(data.values())
        # normalize counts
        counts = np.sum(values)
        values = np.array(values) / counts
        # re-sample the values based on the initial distribution
        values_new = np.random.choice(keys, size=counts, replace=True, p=values)
        # count the occurrences to generate re-sampled dictionary
        values_counted = Counter(values_new)
        # convert to standard dictionary
        data_new = {}
        for bitstring in values_counted:
            data_new[str(bitstring)] = values_counted.get(bitstring)
        return data_new
