...
######################################################################################
# Modifications:                                                                     #
# - allows the use of Qiskit Session                                                 #
# - introduces the submit() method, that submits jobs in a non-blocking fashion      #
# - implement the retrieve() method that retrieves job results in a blocking fashion #
######################################################################################
...
from qiskit_ibm_runtime import Session  # enable qiskit sessions
...
class BaseExperiment(ABC, StoreInitArgs):
    ...
    def __init__(
        ...
        Args:
            ...
            session: Optional, the session to run the experiment within.
    ...
    """
    Modified: This method is an altered version of run(), it submits the jobs in a non-blocking fashion and returns the
    job_ids, which can then be passed to retrieve()
    """
    def submit(
        self,
        session: Optional[Session] = None,
        ...
        **run_options,
        ...
        """Submit an experiment in a non-blocking fashion.

        Args:
            session: Optional run inside a Qiskit Session
            backend: Optional, the backend to run on. Will override existing backend settings.
            sampler: Optional, the sampler to run the experiment on.
                      If None then a sampler will be invoked from previously
                      set backend
        ...
        """
        Modified: # submit and return jobs (non-blocking), runs within session if provided, else individually on backend
        """
        jobs = experiment._run_jobs(transpiled_circuits, sampler=sampler, session=session, **run_opts)
        return jobs

    """
    Modified: This method is an altered version of run(), it retrieves the jobs in a blocking fashion from the job_ids
    that are output from submit()
    """
    def retrieve(
        self,
        jobs,
        ...
        **run_options,
        ...
        """Evaluate an already executed experiment and perform analysis.

        Args:
            jobs: ho handles of the already submitted jobs.
            backend: Optional, the backend to run on. Will override existing backend settings.
            sampler: Optional, the sampler to run the experiment on.
                      If None then a sampler will be invoked from previously
                      set backend
        ...
        # collect already submitted / running / terminated jobs
        experiment_data.add_jobs(jobs, timeout=timeout)
    ...
    def _run_jobs(
        self, circuits: List[QuantumCircuit], sampler: BaseSamplerV2 = None, session: Session = None, **run_options
    ) -> List[Job]:
        ...
        if not self._backend_run:
            if sampler is None:
                # instantiate a sampler from the backend / the session
                if session is None:
                    # print(f'Running on backend {self.backend}.')
                    sampler = Sampler(self.backend)
                else:
                    # print(f'Running within session {session}.')
            ...
            # allows for inspection of jobs before submitting to IQM Resonance
            # jobs = [self.backend.create_run_request(circs, **run_options) for circs in job_circuits]
