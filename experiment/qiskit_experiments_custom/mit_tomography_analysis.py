...
####################################################
# Modifications:                                   #
# - import custom CompositeAnalysis                #
# - propagate `resample` flag to CompositeAnalysis #
####################################################
...
from qiskit_experiments.library.tomography.basis.pauli_basis import PauliMeasurementBasis
...
class MitigatedTomographyAnalysis(CompositeAnalysis):
    ...
    """
    Modified: Added flag to allow for cluster-resampling, propagate to CompositeAnalysis._component_experiment_data
    """
    def __init__(self, roerror_analysis="default", tomography_analysis="default", resample: bool = False):
    ...
    def _run_analysis(self, experiment_data):
        ...
        """
        This function is called num_physical_wires times for baseline experiments (with 2+3=5 data points)
        This function is called num_patches times for experiments (2+3^n data pointes)

        experiment_data.data() contains results for each data point (i.e. list of size 5 for baseline)
        each element is a dictionary, 'counts' contains the bistring counts
        """

        roerror_analysis, tomo_analysis = self._analyses
        """
        Modified: Propagate `resample` flag
        """
        roerror_data, tomo_data = self._component_experiment_data(experiment_data, resample=self.resample)
